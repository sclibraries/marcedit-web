...
- Service status: the isolated `task-180-marcedit-web` container was healthy
  and `/_stcore/health` returned `ok`.
...
- Browser-control availability: unavailable. Two required discovery attempts
  ...
  server, which that skill explicitly forbids as a substitute.
