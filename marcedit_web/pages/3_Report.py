"""Report — rollups across the loaded batch + per-record summary table.

Reads from `st.session_state.records` and walks the batch once, building
`RecordSnapshot`s and aggregating with `collections.Counter`. The page
answers two questions:

* **Across the batch.** Format breakdown, top tags, missing-field counts
  (e.g. "12 of 100 records have no 856"), URL-domain distribution.
* **Per record.** A sortable / searchable table with identifier, title,
  format, leader 06/07, and total tag count.
"""
...
if not session.has_upload():
    st.info(
        "Upload a `.mrc` file from the **Home** page first. Report walks "
...
# Missing-field rollup -----------------------------------------------------

st.markdown("**Missing-field rollup**")
default_check = [t for t in ("001", "245", "856") if t in tag_counter or t in {"001", "245", "856"}]
check_tags = st.multiselect(
    "Check for records missing these tags",
    options=sorted(set(tag_counter) | {"001", "245", "856"}),
    default=default_check,
    help=(
        "For each selected tag, count the records where that tag does not "
...
if check_tags:
    ...
    for tag in check_tags:
        missing = sum(1 for snap in snapshots if snap.tags_present.get(tag, 0) == 0)
        rows.append({
            "tag": tag,
            "missing": missing,
            "of total": total,
...
with col_left:
    ...
    if tag_counter:
        ...
        st.dataframe(top_tags, hide_index=True, use_container_width=True, height=280)
    else:
...
with col_right:
    ...
    if url_domain_counter:
        ...
        st.dataframe(url_df, hide_index=True, use_container_width=True, height=280)
    else:
