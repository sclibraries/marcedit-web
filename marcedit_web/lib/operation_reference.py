...
_QUICK_OVERVIEW = (
    "Tasks → Quick changes uses one **Quick operation** dropdown. It combines Find "
    "and replace, the nine Common field changes below, and the existing specialized "
    "Quick cleanups. Each choice is a one-operation edit: read its summary, "
    "Preview, then Apply "
    "the same current preview. Only the selected operation's controls are shown. "
    "Existing fields are "
    ...
    "operation requires a new Preview; harmless form values remain available when "
    "you return to an operation."
)
