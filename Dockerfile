...
HEALTHCHECK --interval=15s --timeout=3s --start-period=10s --retries=3 \
    CMD python -m marcedit_web.ops.health \
    && python -c "import urllib.request, sys; sys.exit(0 if urllib.request.urlopen('http://localhost:8501/_stcore/health').read().startswith(b'ok') else 1)"
